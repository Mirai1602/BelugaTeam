...
class CalculadoraMatricesAvanzada(QWidget):
    ...
    def init_ui(self):
        ...
        conversiones = QGroupBox("Conversiones")
        ...
        conversion_layout.addWidget(self.combo_conversion)

        self.txt_conversion = QTextEdit()
        self.txt_conversion.setPlaceholderText("Ingrese el número aquí...")
        self.txt_conversion.setMaximumHeight(60)
        conversion_layout.addWidget(self.txt_conversion)

        btn_convertir = QPushButton("Convertir")
        btn_convertir.clicked.connect(self.ejecutar_conversion)
        conversion_layout.addWidget(btn_convertir)

        self.lbl_conversion_resultado = QLabel("Resultado: ")
        conversion_layout.addWidget(self.lbl_conversion_resultado)

        conversiones.setLayout(conversion_layout)
        layout_principal.addWidget(conversiones)
    ...
    def ejecutar_conversion(self):
        try:
            valor = self.txt_conversion.toPlainText().strip()
            opcion = self.combo_conversion.currentText()

            if opcion == "Decimal → Binario":
                resultado = Conversiones.decimal_a_binario(valor)

            elif opcion == "Binario → Decimal":
                resultado = Conversiones.binario_a_decimal(valor)

            elif opcion == "Decimal → Hexadecimal":
                resultado = Conversiones.decimal_a_hexadecimal(valor)

            elif opcion == "Hexadecimal → Decimal":
                resultado = Conversiones.hexadecimal_a_decimal(valor)

            else:
                return

            self.lbl_conversion_resultado.setText(
            ...
            QMessageBox.critical(
                self,
                "Error",
                f"Ocurrió un error durante la conversión: {e}"
            )
